...
function App() {
  ...
  const [cartAddOns, setCartAddOns] = useState<Record<string, AddOn[]>>(() => { try { return JSON.parse(localStorage.getItem("olive-clay-addons") || "{}"); } catch { return {}; } });
  ...
  const liveMenuItems = menuQuery.data?.length ? menuQuery.data.map((item) => ({
    id: String(item.id), category: item.category, image: item.imageUrl, price: String(item.price), discountPrice: item.discountPrice ? String(item.discountPrice) : null, rating: item.rating,
    vegan: Boolean(item.isVegan), popular: Boolean(item.isPopular), available: Boolean(item.isAvailable), addOns: parseAddOns(item.addOnsJson),
  ...
  const effectivePrice = (item: { price: string; discountPrice?: string | null }) => Number(item.discountPrice || item.price);
  const sortedItems = useMemo(() => [...filteredItems].sort((a, b) => sortMode === "priceAsc" ? effectivePrice(a) - effectivePrice(b) : sortMode === "priceDesc" ? effectivePrice(b) - effectivePrice(a) : sortMode === "popular" ? Number(b.popular) - Number(a.popular) || Number(b.rating) - Number(a.rating) : 0), [filteredItems, sortMode]);
  const cartItems = liveMenuItems.filter((item) => cart[item.id]);
  const cartCount = Object.values(cart).reduce((sum, quantity) => sum + quantity, 0);
  const cartTotal = cartItems.reduce((sum, item) => sum + (effectivePrice(item) + (cartAddOns[item.id] || []).reduce((addOnSum, addOn) => addOnSum + addOn.price, 0)) * (cart[item.id] || 0), 0);
  useEffect(() => { localStorage.setItem("olive-clay-cart", JSON.stringify(cart)); }, [cart]);
  useEffect(() => { localStorage.setItem("olive-clay-addons", JSON.stringify(cartAddOns)); }, [cartAddOns]);
  ...
  const toggleAddOn = (id: string, addOn: AddOn) => setCartAddOns((current) => { const selected = current[id] || []; const exists = selected.some((entry) => entry.nameAr === addOn.nameAr); return { ...current, [id]: exists ? selected.filter((entry) => entry.nameAr !== addOn.nameAr) : [...selected, addOn] }; });
  ...
  return (
    <div className={`site-shell ${isNight ? "night-mode" : ""}`} dir={isArabic ? "rtl" : "ltr"}>
      <header className="topbar">
        <a className="brand" href="#top" aria-label="مساء cafe"><span className="brand-masaa">مساء</span><em>cafe</em></a>
        <nav className={`desktop-nav ${menuOpen ? "is-open" : ""}`}>
          <a href="#menu">{t.navMenu}</a><a href="#visit">{t.navVisit}</a>
        </nav>
        <div className="top-actions">
          <div className="language-switcher" aria-label="Language"><button className={locale === "ar" ? "active" : ""} onClick={() => switchLocale("ar")}>AR</button><span>|</span><button className={locale === "en" ? "active" : ""} onClick={() => switchLocale("en")}>EN</button></div>
          <button className="account-button" onClick={() => startLogin()} aria-label={isArabic ? "الحساب" : "Account"}><UserRound size={16} /></button>
      ...
      <div className={`qr-banner ${scannedTable ? "table-session" : ""}`}><img src={`https://api.qrserver.com/v1/create-qr-code/?size=88x88&data=${encodeURIComponent(window.location.origin)}`} alt="QR code" /><span>{scannedTable ? `${isArabic ? "طلب الطاولة" : "Table"} ${scannedTable}` : t.qrLabel}</span><a href="#menu">{t.navMenu} <ArrowUpRight size={13} /></a></div>

      <main id="top">
        <section className="hero-section">
          <div className="hero-socials" aria-label="Social media"><a href="#instagram" aria-label="Instagram"><Instagram size={17} /></a><a href="#whatsapp" aria-label="WhatsApp"><MessageCircle size={17} /></a><a href="#tiktok" aria-label="TikTok" className="social-letter">♪</a><a href="#x" aria-label="X" className="social-letter">𝕏</a></div>
        ...
        <section className="menu-section" id="menu">
          ...
          <div className="menu-groups">{categories.filter(({ key }) => key !== "all" && (activeCategory === "all" || activeCategory === key)).map((group) => { const groupItems = sortedItems.filter((item) => item.category === group.key); if (!groupItems.length) return null; return <section className={`menu-category-group category-${group.key}`} key={group.key}><header className="category-heading"><span>0{categories.findIndex((entry) => entry.key === group.key)}</span><h3>{t[group.label]}</h3><b>{groupItems.length} {isArabic ? "صنف" : "items"}</b></header><div className="menu-grid">{groupItems.map((item, index) => { const [title, description] = item[locale]; const liked = favourites.includes(item.id); const discountPrice = Number((item as any).discountPrice || 0); return <article className="dish-card" key={item.id} style={{ "--delay": `${index * 35}ms` } as React.CSSProperties} onClick={() => setSelectedItem(item)}><div className="dish-image-wrap"><img src={item.image} alt={title} /><div className="image-overlay" /><button className={`heart-button ${liked ? "liked" : ""}`} onClick={(event) => { event.stopPropagation(); toggleFavourite(item.id); }} aria-label={t.order}><Heart size={18} fill={liked ? "currentColor" : "none"} /></button>{item.popular && <span className="popular-tag"><Star size={12} fill="currentColor" />{t.popular}</span>}<span className="available-tag"><span />{t.available}</span><span className="image-hint">{locale === "ar" ? "عرض التفاصيل" : locale === "fr" ? "Voir les détails" : "View details"}</span></div><div className="dish-content"><div className="dish-title-row"><h3>{title}</h3><span className={`price ${discountPrice ? "discounted" : ""}`}>{discountPrice > 0 && <del>{item.price}</del>}<small>SAR</small> {discountPrice || item.price}</span></div><p>{description}</p><div className="dish-meta"><span className="rating"><Star size={13} fill="currentColor" /> {item.rating}</span>{item.vegan && <span className="vegan-tag"><Leaf size={13} /> {t.vegan}</span>}<button className="tiny-action" onClick={(event) => { event.stopPropagation(); toggleFavourite(item.id); }}>{liked ? <Check size={14} /> : <Heart size={14} />} {liked ? (isArabic ? "محفوظ" : locale === "fr" ? "Ajouté" : "Saved") : t.order}</button></div><button className="add-cart-button" onClick={(event) => { event.stopPropagation(); addToCart(item.id); }}>{t.addToCart}<ShoppingCart size={15} /></button></div></article>; })}</div></section>; })}</div>
      ...
      {selectedItem && <div className="modal-backdrop dish-detail-backdrop" role="presentation" onClick={() => setSelectedItem(null)}><div className="dish-detail-modal" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setSelectedItem(null)} aria-label={t.closeDetails}><X size={20} /></button><img className="detail-image" src={selectedItem.image} alt={selectedItem[locale][0]} /><div className="detail-body"><span className="section-kicker">{selectedItem.popular ? t.popular : t.tonight}</span><div className="detail-title-row"><h2>{selectedItem[locale][0]}</h2><span className={`price ${selectedItem.discountPrice ? "discounted" : ""}`}>{selectedItem.discountPrice && <del>{selectedItem.price}</del>}<small>SAR</small> {selectedItem.discountPrice || selectedItem.price}</span></div><p>{selectedItem[locale][1]}</p><h3>{t.ingredientsLabel}</h3><div className="ingredient-pills">{(selectedItem.ingredients || ["Extra virgin olive oil", "Seasonal herbs", "Toasted spices"]).map((ingredient: string) => <span key={ingredient}>{ingredient}</span>)}</div>{selectedItem.addOns?.length > 0 && <><h3 className="addons-title">{t.addOns}</h3><div className="addon-options">{selectedItem.addOns.map((addOn: AddOn) => { const selected = (cartAddOns[selectedItem.id] || []).some((entry) => entry.nameAr === addOn.nameAr); const label = locale === "ar" ? addOn.nameAr : locale === "fr" ? (addOn.nameFr || addOn.nameEn || addOn.nameAr) : (addOn.nameEn || addOn.nameAr); return <button className={selected ? "selected" : ""} key={addOn.nameAr} onClick={() => toggleAddOn(selectedItem.id, addOn)}>{selected && <Check size={13} />}{label}<b>+{addOn.price} SAR</b></button>; })}</div></>}<button className="primary-cta detail-add" onClick={() => { addToCart(selectedItem.id); setSelectedItem(null); }}>{t.addToCart}<ShoppingCart size={17} /></button></div></div></div>}
      ...
      {cartOpen && <div className="modal-backdrop cart-backdrop" role="presentation" onClick={() => setCartOpen(false)}><aside className="cart-drawer" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><div className="cart-header"><div><span className="section-kicker">OLIVE & CLAY</span><h2>{t.cart}</h2></div><button className="modal-close" onClick={() => setCartOpen(false)} aria-label={t.close}><X size={20} /></button></div>{cartItems.length ? <><div className="cart-list">{cartItems.map((item) => { const extras = cartAddOns[item.id] || []; const linePrice = effectivePrice(item) + extras.reduce((sum, addOn) => sum + addOn.price, 0); return <div className="cart-line" key={item.id}><img src={item.image} alt="" /><div className="cart-line-info"><strong>{item[locale][0]}</strong><small><span className="price"><small>SAR</small> {linePrice}</span> × {cart[item.id]}</small>{extras.length > 0 && <em>{extras.map((addOn) => addOn.nameAr).join("، ")}</em>}</div><div className="quantity-controls"><button onClick={() => changeCartQuantity(item.id, -1)}><Minus size={13} /></button><b>{cart[item.id]}</b><button onClick={() => changeCartQuantity(item.id, 1)}><Plus size={13} /></button></div></div>; })}</div><div className="cart-total"><span>{t.total}</span><strong><small>SAR</small> {cartTotal}</strong></div><a className="primary-cta cart-submit" href={scannedTable ? `/checkout?table=${encodeURIComponent(scannedTable)}` : "/checkout"}>{t.cart} <ArrowUpRight size={17} /></a></> : <div className="cart-empty"><ShoppingCart size={32} /><p>{t.cartEmpty}</p><button className="text-link" onClick={() => setCartOpen(false)}>{t.explore}</button></div>}</aside></div>}
