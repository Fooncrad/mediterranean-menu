...
type OrderType = "table" | "reservation" | "takeaway" | "delivery" | "room_service";
type CartMap = Record<string, number>;
type CartAddOns = Record<string, Array<{ nameAr: string; price: number }>>;
...
const orderTypes: { value: OrderType; label: string; hint: string; icon: typeof Utensils }[] = [
  { value: "table", label: "طلب على الطاولة", hint: "اطلب مباشرة إلى رقم طاولتك", icon: Utensils },
...
export default function Checkout() {
  ...
  const [cartAddOns] = useState<CartAddOns>(() => { try { return JSON.parse(localStorage.getItem("olive-clay-addons") || "{}"); } catch { return {}; } });
  const menuQuery = trpc.menu.list.useQuery();
  const orderMutation = trpc.orders.create.useMutation({ onSuccess: () => { localStorage.removeItem("olive-clay-cart"); localStorage.removeItem("olive-clay-addons"); setSubmitted(true); } });
  const items = useMemo(() => (menuQuery.data ?? []).filter((item) => cart[String(item.id)]).map((item) => { const addOns = cartAddOns[String(item.id)] || []; return { id: String(item.id), title: item.nameAr, price: (item.discountPrice ?? item.price) + addOns.reduce((sum, addOn) => sum + addOn.price, 0), quantity: cart[String(item.id)], addOns: addOns.map((addOn) => ({ name: addOn.nameAr, price: addOn.price })) }; }), [menuQuery.data, cart, cartAddOns]);
  const subtotal = items.reduce((sum, item) => sum + item.price * item.quantity, 0);
  const deliveryFee = orderType === "delivery" ? deliveryZones.find((zone) => zone.value === deliveryZone)?.fee ?? 0 : 0;
  const taxAmount = Math.round((subtotal + deliveryFee) * 0.15);
  const total = subtotal + deliveryFee + taxAmount;

  const submit = (event: React.FormEvent) => {
    event.preventDefault();
    orderMutation.mutate({ orderType, customerName, customerPhone: customerPhone || undefined, address: orderType === "delivery" ? address : undefined, deliveryZone: orderType === "delivery" ? deliveryZone : undefined, roomNumber: orderType === "room_service" ? roomNumber : undefined, tableNumber: orderType === "table" ? tableNumber : undefined, reservationAt: orderType === "reservation" && reservationDate ? new Date(`${reservationDate}T${reservationTime}:00`) : undefined, guestCount: orderType === "reservation" ? Number(guestCount) : undefined, items, total: subtotal });
  };
